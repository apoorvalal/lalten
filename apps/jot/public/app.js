(() => {
  ...
  function initListPage() {
    ...
    newNoteButton.addEventListener("click", async () => {
      ...
      window.location.href = withBase(`/notes/${payload.note.id}`);
    });
    ...
    noteList.addEventListener("click", async (event) => {
      ...
      if (!row) return;
      window.location.href = withBase(`/notes/${row.dataset.noteId}`);
    });
  ...
  function initNotePage(isPublic) {
    ...
    function initCollabEditor() {
      ...
      import(withBase("/static/collab-editor.js")).then(({ createCollabEditor }) => {
    ...
    function connectWebSocket(refsArg, publicMode) {
      ...
      const wsUrl = `${protocol}//${location.host}${withBase(`/`)}?${param}`;
  ...
  function toggleSharePopover(refs) {
    ...
    const shareUrl = `${location.origin}${withBase(`/s/${state.note?.shareId || ""}`)}`;
    popover.innerHTML = `
  ...
  function openAgentModal(refs) {
    ...
    if (isOwnerView) {
      ...
      const shareUrl = `${baseUrl}${withBase(`/s/${state.note?.shareId || shareId}`)}`;
      lines.push(
